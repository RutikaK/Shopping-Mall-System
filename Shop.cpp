...
class customer {					//customer class
	...
	void getData() {				//getting input from customer
		...
		cout<<"ARE YOU SHOPPING WITH US FOR THE FIRST TIME (press '1' for 'yes', '0' for 'no'): "<<endl;
		cin>>cus;
		if(cus==1){
...
void single_llist::display()
{
	...
	cout<<endl<<setw(7)<<left<<"SR NO."<<setw(20)<<"ITEMS"<<setw(10)<<"PRICE"<<endl;
	while (temp != NULL) {
	count ++;
	...
	while(1) {
		...
		if(choice==1) {
			...
			if(choice1==1) {
				...
				cout<<"Enter '3' for 'water flosser: 227 Rs.' "<<endl;
				cin >> choice2;
				if(choice2 == 1)
